...
- (void)startScan {
    ...
    NSLog(@"address:%@", self.localAddress);
    if ([self.localAddress isEqualToString:@"error"]) {
    ...
    if ([self isIpAddressValid:self.localAddress] && (a.count == 4) && (b.count == 4)) {
        for (int i = 0; i<a.count; i++) {
            long and = [[a objectAtIndex:i] integerValue] & [[b objectAtIndex:i] integerValue];
...
- (void)pingAddress {
    ...
    weakSelf.currentHostAddress++;
    NSLog(@"ping address:%@ hostAddress:%ld", weakSelf.baseAddress, (long)weakSelf.currentHostAddress);
    NSString *address = [NSString stringWithFormat:@"%@%ld", weakSelf.baseAddress, (long)weakSelf.currentHostAddress];
    [SimplePingHelper ping:address target:self sel:@selector(pingResult:)];
    if (weakSelf.currentHostAddress>=254) {
...
- (void)pingResult:(NSNumber*)success {
    __weak typeof(self) weakSelf = self;
    weakSelf.timerIterationNumber++;
    if (success.boolValue) {
        NSLog(@"SUCCESS");
        NSString *deviceIPAddress = [[[[NSString stringWithFormat:@"%@%ld", weakSelf.baseAddress, (long)weakSelf.currentHostAddress] stringByReplacingOccurrencesOfString:@".0" withString:@"."] stringByReplacingOccurrencesOfString:@".00" withString:@"."] stringByReplacingOccurrencesOfString:@".." withString:@".0."];
        ...
        [weakSelf.delegate scanLANDidFindNewAdrress:deviceIPAddress havingHostName:deviceName];
    }
    else {
        NSLog(@"FAILURE");
    }
    if (weakSelf.timerIterationNumber+weakSelf.baseAddressEnd>=254) {
        [weakSelf.delegate scanLANDidFinishScanning];
    }
}
